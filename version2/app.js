...
function syncConsentUi() {
  const hasContext = toggleHistory.checked || toggleAdvisor.checked || toggleGroup.checked;
...
document.querySelectorAll("[data-home-action]").forEach((button) => {
  button.addEventListener("click", () => {
    const scenarioId = button.dataset.homeAction;
...
function wireGroupChip(chip) {
  chip.addEventListener("click", () => {
    ...
    toggleGroup.checked = state.selectedGroups.length > 0;
    syncConsentUi();
  });
}
