...
function buildContext({ scenarioId, toggles, history, authorizedContext, profileContext }) {
  const scenario = scenarioContexts[scenarioId] || scenarioContexts["repair-message"];
...
function buildUserPrompt({ scenarioId, userInput, toggles, history, authorizedContext, profileContext }) {
  const contextLines = buildContext({ scenarioId, toggles, history, authorizedContext, profileContext });
  return [
    `场景 ID：${scenarioId}`,
    `用户输入：${userInput}`,
    "已知上下文：",
...
function normalizeModelResult(result) {
  ...
  return {
    ...
    inspector: {
      ...
      signals:
        Array.isArray(inspector.signals) && inspector.signals.length
...
async function generateLiveResponse(body) {
  if (!OPENAI_API_KEY) {
    const error = new Error("缺少 OPENAI_API_KEY。请先在部署环境中配置后再发送。");
  ...
  const response = await fetch(`${OPENAI_BASE_URL}/chat/completions`, {
    ...
    if (error.name === "AbortError") {
      const timeoutError = new Error(
        `上游模型在 ${UPSTREAM_TIMEOUT_MS / 1000} 秒内没有返回，请检查 BASE_URL、MODEL 或稍后重试。`
      );
